...
void app_main()
{
    ...
    sdmmc_card_t* card;
    esp_err_t retSDCARD = esp_vfs_fat_sdmmc_mount("/sdcard", &host, &slot_config, &mount_config, &card);
    ...
    FILE* f = fopen("/sdcard/mbtr.pcm", "r");
    if (f == NULL) {
    ...
    m_sample = open("/sdcard/pla.pcm", O_RDONLY);
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
    ...
    if (eir) {
        ...
        if (strcmp((char *)s_peer_bdname, "ActivBuds S-250") != 0) {
            return;
        }
